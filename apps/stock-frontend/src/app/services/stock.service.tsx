...
export async function fetchSearchStocks(searchInput: string) {
    const response = await axios.get(`https://financialmodelingprep.com/api/v3/search?query=${searchInput}&limit=10&apikey=${import.meta.env.VITE_STOCK_API_KEY}`);
    return response.data;
}

export async function fetchSelectedStockQuote(stockData: StockInfo): Promise<StockQuote> {
    const response = await axios.get(`https://financialmodelingprep.com/api/v3/quote-short/${stockData.symbol}?apikey=${import.meta.env.VITE_STOCK_API_KEY}`);
